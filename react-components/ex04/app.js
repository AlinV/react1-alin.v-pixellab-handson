...
console.warn(`Prin aceeasi metoda, afiseaza o lista inversata cu numele
...
Object.entries(person.friends).forEach((friendEntry) => {
  ...
  const word = ageDiff === 1 ? 'an' : 'ani';

  console.log(
    `Diferenta de varsta intre ${name} si ${person.name} este de ${ageDiff} ${word}.`,
  );
});
...
Object.entries(person.skills).map((skillEntry) => {
...
console.warn(`Afiseaza propozitia: “Prietenii mei sunt Larry Larryson, Steven Stevenson
